...
| **Fixed-vs-flexible listing caveat** | not stated on a 9.0 page | stated verbatim in the spec: this endpoint *"does not return flexible segments"* and the search API is the workaround | **[SPEC-9.1]** — behaviour assumed identical in 9.0 |
...
| Edge platform | Edge Host Affinity; edge install/config via vCenter; Gateway Firewall disabled by default for greenfield | new simplified Edge Node / Edge Cluster UI workflow; Bare Metal Edge NIC support (Broadcom 574X/575X, Mellanox CX6 LX); control-plane packet prioritisation on edge uplinks; *"VLAN and MTU pre-check for the Edge node"* during Edge VM deployment; Bare Metal Edge **import** into VCF | **[DOC]** |
