# vSphere inventory and VM lifecycle — VCF 9.0

**Applies to:** vCenter **9.0.0.0** (build 24755230) and ESX **9.0.0.0** (build 24755229),
the versions in the VCF 9.0 Bill of Materials.
**Do not apply this file to VCF 9.1.** Use `../9.1/inventory-and-vms.md` for 9.1 and
`../deltas.md` for the change list.

> **The hypervisor is called ESX in 9.0, not ESXi.** The BOM row is literally
> *"VMware ESX"*. The 9.0 product-support-notes page uses both spellings interchangeably
> and contains no explicit "renamed from ESXi" statement — so the *product name* is ESX,
> but a formal rename statement is `UNVERIFIED`.

## Provenance of everything below

| Tag | Meaning |
|---|---|
| **[SPEC]** | The exact `method + path` was found in `research/spec-inventory/9.0__vsphere-automation.ops.json` — machine-extracted from `specifications/vsphere/openapi/automation/vcenter.yaml` at git tag `9.0.0.0` of `github.com/vmware/vcf-api-specs` (`info.version: 9.0.0.0`, `servers[0].url: https://{host}/api`, 1,275 operations). Schema and description quotes come from that same file. Strongest evidence available. |
| **[DOC]** | Verified only from version-pinned Broadcom prose (VCF 9.0 programming guide, 9.0 release notes, 9.0 product support notes). |
...
evidence asymmetry to warn about here. A `[SPEC]` tag in this file is evidence about **9.0**;
the 9.1 file carries its own, independently extracted.
...
- [**Prerequisites**](#prerequisites) — **read before any write**
  ...
  - [P2 — **the federated-login gate**](#p2--your-credential-can-actually-create-a-session-the-90-login-gate)
...
- **Must be true:** an `https://<vcenter>` endpoint on port **443**, its certificate chain
  in your trust store, and every request under **`/api`**. Verbatim from the VCF 9.0
...
- **9.1 difference:** none. The 9.1 programming guide repeats the wording verbatim and the
  9.1 spec declares the same `servers[0].url`.

### P2 — Your credential can actually create a session (the 9.0 login gate)

- **Must be true:** the identity you are authenticating with is acceptable to a vCenter
  that **blocks non-federated username/password logins**. Verbatim from the VCF 9.0
  product support notes, under removals: *"Blocked non-federated username/password logins
  to vCenter: vCenter 9.0 blocks logins with just a user name and password, which might
  sometimes allow bypassing the federated provider domain."* **[DOC]**
...
- **The spec's own signal.** The 9.0 spec declares three security schemes **[SPEC]**:
  | Scheme | Type | Notes |
  |---|---|---|
...
- **Also removed in 9.0, same page, same blast radius [DOC]:** Integrated Windows
  Authentication (IWA), SSPI, smart card and RSA SecurID. Any 9.0 client that relied on one
  of those is dead, not degraded.
- **9.1 difference:** the 9.0 removal statement is not restated in the 9.1 support notes,
  and the 9.1 release notes add *"OAuth 2.0 token support"* to vCenter **[DOC]**. Treat the
  gate as **still in force** in 9.1 unless the customer's build says otherwise — a removal
  does not un-remove. See `../deltas.md`, which also records that the OAuth token-exchange
  *endpoint* is present at both tags and is therefore not itself the 9.1 change.
...
- **Session idle timeout is not stated in the spec** and no 9.0 page retrieved gives a
  ...
  presumed lifetime.
- **9.1 difference:** none. Identical three operations, identical header.
...
- **9.1 difference:** the privilege strings and the `authorization` operation set are
  identical at the 9.1 tag (18 operations at both). The 9.1 spec adds a FIPS-modules read
  elsewhere but nothing in `authorization`.
...
- **9.1 difference:** none. `Vcenter.VM.CreateSpec`, `Vcenter.VM.PlacementSpec`,
...
- **9.1 difference:** identical caps at the 9.1 tag. The 9.1 release notes describe a new
  **Query API** for *"a fast, flexible, and scalable way to retrieve vSphere inventory
  data"* with server-side filtering and pagination — but **no such path exists in the 9.1
  ops inventory** and its endpoints are `UNVERIFIED`. See `../deltas.md`.
...
`GET /sdk/vim25/9.0.0.0/VirtualMachine/vm-495/config`,
`POST /sdk/vim25/9.0.0.0/SessionManager/{moId}/Login`. Its login is `SessionManager.Login`,
...
Three operations, both versions, all **[SPEC — `9.0__vsphere-automation.ops.json`]**:
...
**202** and defers the work. **144 operations** carry the `vmw-task=true` suffix in the 9.0
spec. **[SPEC]**
...
> machine"*, not "task identifier". Across the whole 9.0 spec there are 144 `202`
> descriptions, 59 of them just *"Success!"*, and **not one mentions a task**. So the spec **declares** the async variant
...
All **[SPEC — `9.0__vsphere-automation.ops.json`]**. Paths shown relative to `/api`.
...
### Hybrid Linked Mode (`/hvc/*`) — present in 9.0, **removed in 9.1**

Nine operations, all **[SPEC — `9.0__vsphere-automation.ops.json`]**, paths relative to `/api`:
`GET·POST /hvc/links`, `GET·DELETE /hvc/links/{link}`,
`POST /hvc/links/{link}?action=delete` (`Vcenter.Hvc.Links_deleteWithCredentials`),
`GET·PUT /hvc/management/administrators`, and
`POST /hvc/management/administrators?action=add|remove`.

HLM joins a remote vCenter's SSO domain — and therefore its inventory — to the local one, so
it is inventory-scope even though it looks like an auth feature.

> **All nine are gone at the 9.1 tag.** They are the *only* nine operations removed from
> `vsphere-automation` between 9.0.0.0 and 9.1.0.0, and there is no renamed successor.
> Anything you build on `/hvc/*` today breaks on upgrade. Raise it before someone writes a
> cross-vCenter inventory tool on it. See `../deltas.md`.
>
> Do not conflate this with **Enhanced Linked Mode**, which was *deprecated* (not removed) at
> 9.0 in favour of grouping under VCF Operations **[DOC]**. Different feature.
...
`Vcenter.Datastore.Type` still lists `VVOL`, but **vVols are deprecated in 9.0** per the
product support notes **[DOC]**. Present in the enum is not the same as recommended.

---

## VM lifecycle

All **[SPEC — `9.0__vsphere-automation.ops.json`]**. Paths relative to `/api`.
...
> **9.0 hardware note [DOC]:** ESX 9.0 introduces **virtual hardware version 22**
> (960 logical processors, 960 cores per socket, NVMe 1.4, 4KN VMDK, Intel TDX, AMD SEV-SNP).
> `hardware_version` is a `Vcenter.Vm.Hardware.Version` value; the concrete enum member names
> were not extracted and are `UNVERIFIED` — read them from the spec's
> `Vcenter.Vm.Hardware.Version` schema before hard-coding one.
...
(`Vcenter.Vm.Guest.Customization_check`) — the *"new VM-customization pre-check API"* the 9.0
release notes advertise **[DOC]**. Run the check before the set.
...
|---|---|
| 401 on step 0 with a credential you trust | **P2** — vCenter 9.0 blocks non-federated username/password logins. Not a typo. Route to `vcf-foundation`. |
...
| Topic | Paths | Skill |
|---|---|---|
| Content library, library items, VM template items | `/api/content/*` (72 ops at 9.0), `/api/vcenter/vm-template/library-items/*` | `vsphere-content-tags-policies` |
| Tags and categories | `/api/cis/tagging/*` (30 ops at 9.0), `/api/vcenter/tagging/{associations,categories,tags}` | `vsphere-content-tags-policies` |
...
| ESX images, drafts, depots, remediation, configuration profiles | `/api/esx/settings/*` (**339 ops at 9.0**) | `vsphere-lifecycle-vlcm` |
...
> whether vLCM lives at `/api/esx/settings/...` or `/api/esx-settings/...`. The 9.0 ops
> inventory settles it: the paths are **`/esx/settings/...`** (slash), 339 operations.
...
- **Session idle timeout.** No number in the spec; no 9.0 page retrieved states one. Build
...
- **Tag and category operation semantics.** The dossier flags the tag/category operation
  ...
  under `/cis/tagging`), but their bodies and semantics were not researched here and belong
...
- **Whether existing Hybrid Linked Mode links survive a 9.0 → 9.1 upgrade**, and what if
  anything replaces `/hvc/*`. The removal at the 9.1 tag is machine-confirmed; the
  consequence is not documented on any page retrieved.
